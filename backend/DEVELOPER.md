...
* `DUO_PAYPAL_PLAN_IDS` - A comma-separated list of the PayPal billing plans to offer, e.g. the weekly, monthly and yearly plans. Subscriptions on plans that are no longer listed keep working until they lapse.
* `DUO_PAYPAL_WEBHOOK_ID` - The ID of the webhook that posts subscription events to `/paypal/webhook`.

`./create-paypal-plans.sh` creates the weekly ($1.99), monthly ($3.99) and yearly ($30.00) plans and prints the `DUO_PAYPAL_PLAN_IDS` line to use. It reads `DUO_PAYPAL_CLIENT_ID`, `DUO_PAYPAL_CLIENT_SECRET`, `DUO_PAYPAL_API_URL` (set to `https://api-m.sandbox.paypal.com` for the sandbox) and either `DUO_PAYPAL_PRODUCT_ID` or the `DUO_PAYPAL_PLAN_ID` of an existing plan, whose product is reused.

The mobile apps buy Gold through RevenueCat, which offers whatever packages are in the current offering. `./create-revenuecat-packages.sh` makes sure the `$rc_weekly`, `$rc_monthly` and `$rc_annual` packages exist and attaches store products to them. It reads `DUO_REVENUECAT_API_KEY` (a v2 secret key with write access to project configuration), `DUO_REVENUECAT_PROJECT_ID` and, for each store product you have created in App Store Connect or the Play Console, its identifier in `APP_STORE_WEEKLY`, `APP_STORE_MONTHLY`, `APP_STORE_ANNUAL`, `PLAY_STORE_WEEKLY`, `PLAY_STORE_MONTHLY` and `PLAY_STORE_ANNUAL` (Play identifiers take the form `productId:basePlanId`). Leave a variable unset to skip that product, e.g. when the existing weekly product has merely had its store price changed. Prices live in the stores, and RevenueCat's API can't create Web Billing products, so those stay in the dashboard.
