...
The vocabulary (question ids, enum sizes, the country and club lists) is
...
AGE_LO, AGE_HI, AGE_CENTRE, AGE_SCALE = 18.0, 80.0, 30.0, 10.0
...
def _numeric(age: F64Array, height: F64Array) -> tuple[FloatArray, FloatArray]:
    a = np.clip(age, AGE_LO, AGE_HI)
    h = height
    hm = ~np.isnan(h) & (h >= HEIGHT_LO) & (h <= HEIGHT_HI)
    h = np.where(hm, h, HEIGHT_CENTRE)
    num = np.stack([(a - AGE_CENTRE) / AGE_SCALE,
                    (h - HEIGHT_CENTRE) / HEIGHT_SCALE], axis=1)
    mask = np.stack([np.ones(len(a)), hm], axis=1)
...
def who_input(spec: Spec, b: Blocks) -> FloatArray:
    ...
    num, num_mask = _numeric(b.age, b.height_cm)
    return np.concatenate([
        b.answers,
        *cats,
