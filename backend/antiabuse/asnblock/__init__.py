...
(`blocked-asns.json`, ASN -> operator name) of VPN / hosting providers whose
addresses are a recurring source of abuse.

Like `antiabuse.firehol`, lookups fail open: any timeout, connection error, or
malformed response yields no ASNs, i.e. "not blocked". A typical RIPEstat
lookup takes ~0.3 s from the server, so the default timeout adds only a small
margin on top of that; if RIPEstat is slower, we just let the user sign up.
"""
...
# ASN -> operator name. The names are documentation only; membership is
# decided on the ASN alone.
...
def blocked(asns: Iterable[Asn]) -> bool:
    """Whether any of `asns` is on the blocklist."""
    return any(asn in BLOCKED_ASNS for asn in asns)
