...
class TxCursor:
    ...
    async def execute(
        ...
        await self._staleness.note_before(query, params)
    ...
    async def executemany(
        ...
        await self._staleness.note_before(query, None)
    ...
    async def fetchone(self) -> Row | None:
        ...
        if row is not None:
            self._staleness.saw_rows([row])
        return row

    async def fetchall(self) -> list[Row]:
        rows = await self._cur.fetchall()
        self._staleness.saw_rows(rows)
        return rows
...
async def api_tx(
    ...
    async with _pool().connection() as conn, conn.cursor() as raw_cur:
        ...
        yield cur
        await cur._staleness.flush(cur)
