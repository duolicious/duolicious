...
class EvalData:
    ...
    def _mutual_gender_mask(self, a: int, cands: np.ndarray) -> np.ndarray:
        ga = self.gender[a]
    ...
    def _candidates(self, a: int, b: int, k: int) -> np.ndarray:
    ...
    def exposure_candidates(self, a: int) -> np.ndarray:
...
class Scorer:
    ...
    def __init__(self, look: np.ndarray, who: np.ndarray, device: torch.device) -> None:
    ...
    def directed(self, a: np.ndarray, b: np.ndarray) -> np.ndarray:
    ...
    def reciprocal(self, a: np.ndarray, b: np.ndarray) -> np.ndarray:
        return self.directed(a, b) + self.directed(b, a)

    def rank_of_first(self, a: int, cands: np.ndarray, mode: str) -> int:
    ...
    def topk(self, a: int, cands: np.ndarray, k: int, mode: str) -> np.ndarray:
        a_arr = np.full(len(cands), a)
        s = self.reciprocal(a_arr, cands) if mode == "recip" else self.directed(a_arr, cands)
...
def baseline_metrics(f: Features, ed: EvalData, train_a: np.ndarray,
                     train_b: np.ndarray, train_label: np.ndarray,
                     train_replied: np.ndarray) -> dict[str, dict[str, float]]:
    """Popularity / reply-rate / gender-filter baselines on the held-out
    window, using only pre-split interactions."""
    n = f.n
    pos = train_label > 0
    indeg = np.bincount(train_b[pos], minlength=n).astype(float)
    outdeg = np.bincount(train_a[pos], minlength=n).astype(float)
    received = indeg
    replied_to = np.bincount(train_b[pos & train_replied], minlength=n).astype(float)
    reply_rate = (replied_to + 1) / (received + 3)
    skipdeg = np.bincount(train_b[~pos], minlength=n).astype(float)
    pos_rate = (indeg + 1) / (indeg + skipdeg + 2)

    def gmask(a, b):
        return (ed.gender_ok[a, ed.gender[b]] & ed.gender_ok[b, ed.gender[a]]).astype(float)

    out: dict[str, dict[str, float]] = {}
    out["popularity"] = {
        "dir_auc": safe_auc(ed.dir_y, np.log1p(indeg[ed.dir_b])),
        "reply_auc_b2a": safe_auc(ed.rep_y, -np.log1p(indeg[ed.rep_b])),
        "reply_auc_recip": safe_auc(ed.rep_y, np.log1p(indeg[ed.rep_a]) - np.log1p(indeg[ed.rep_b])),
    }
    out["pos_rate_of_b"] = {
        "dir_auc": safe_auc(ed.dir_y, pos_rate[ed.dir_b]),
    }
    out["reply_rate_of_b"] = {
        "reply_auc_b2a": safe_auc(ed.rep_y, reply_rate[ed.rep_b]),
        "reply_auc_recip": safe_auc(ed.rep_y, reply_rate[ed.rep_b] + np.log1p(outdeg[ed.rep_b])),
    }
    out["gender_filter"] = {
        "dir_auc": safe_auc(ed.dir_y, gmask(ed.dir_a, ed.dir_b)),
        "reply_auc_b2a": safe_auc(ed.rep_y, gmask(ed.rep_a, ed.rep_b)),
    }
    return out
...
def disagreement(f: Features, a: np.ndarray, b: np.ndarray, qids: list[int]) -> float:
...
def agreement_probe(sc: Scorer, ed: EvalData, k: int = 50, mode: str = "recip",
    ...
    for a in ed.searchers[:n_searchers]:
        ...
        if len(cands) == 0:
            continue
        top = sc.topk(a, cands, k, mode)
        a_all.append(np.full(len(top), a))
        b_all.append(top)
...
def grouped_auc(group: np.ndarray, y: np.ndarray, s: np.ndarray, min_size: int = 3) -> float:
...
def dynamic_exposure(sc: Scorer, ed: EvalData, lam: float, sd: float, k: int = 50,
                     mode: str = "recip") -> dict[str, float]:
    """Serve searchers one after another, penalising each prospect by
    lam * sd * log1p(times already shown). Reports exposure concentration and
    how far down the un-penalised ranking the shown prospects sit."""
    counts = np.zeros(ed.f.n, dtype=np.float32)
    pct = []
    for a in ed.searchers:
        cands = ed.exposure_candidates(a)
        if len(cands) == 0:
            continue
        a_arr = np.full(len(cands), a)
        s = sc.reciprocal(a_arr, cands) if mode == "recip" else sc.directed(a_arr, cands)
        s2 = s - lam * sd * np.log1p(counts[cands])
        idx = np.argpartition(-s2, min(k, len(s2) - 1))[:k]
        counts[cands[idx]] += 1
        order = np.argsort(-s)
        rank = np.empty(len(s), dtype=np.int64)
        rank[order] = np.arange(len(s))
        pct.append(rank[idx].mean() / len(s))
    pool_counts = counts[ed.exposure_pool]
    srt = np.sort(pool_counts)[::-1]
    top1 = max(1, len(srt) // 100)
    return {
        ...
        "exposure_zero_frac": float((pool_counts == 0).mean()),
        "exposure_max": int(srt[0]),
        "shown_mean_base_rank_pct": float(np.mean(pct)),
    }
