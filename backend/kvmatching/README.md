...
Nothing else is needed. A person's vectors are recomputed in the same
transaction as any change to their answers, profile, clubs or search
preferences (`serviceshared/kvmatching`), because their own `key` half
decides the order of their next search -- a filter change that took a minute
to show up would just look broken. The `kvvectors` cron covers what that
cannot: a newly deployed artifact, which invalidates everyone, and anyone
otherwise missed. It walks `person` oldest-computed-first, so new people sort
to the front, and a batch of 2,000 takes about three seconds.
