...
class TestUnorderedUncachedSearch(unittest.TestCase):
    def test_is_the_ordered_search_minus_the_ordering_tail(self) -> None:
        prefs = maximal_prefs()
        ordered_sql, ordered_params = build_uncached_search(1, 10, 0, prefs)
        unordered_sql, unordered_params = \
            build_unordered_uncached_search(1, 10, 0, prefs)

        self.assertTrue(ordered_sql.startswith(unordered_sql))
        self.assertEqual(ordered_params, unordered_params)

        tail = ordered_sql[len(unordered_sql):]
        self.assertIn('ORDER BY', tail)
