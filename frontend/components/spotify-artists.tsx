...
import { SpotifyArtistItem, spotifyArtistUrl } from '../spotify/spotify';
import { AppTheme, useAppTheme } from '../app-theme/app-theme';
import { themedSurface } from '../app-theme/surface';

// Spotify's icon mark, rendered monochrome in the surrounding text color per
// Spotify's branding guidelines for colored backgrounds. Displaying it
// alongside the artists is required attribution:
...
const inkColor = (appTheme: AppTheme, textStyle?: TextStyle) => {
  const color = StyleSheet.flatten(textStyle)?.color;
  return typeof color === 'string' ? color : appTheme.secondaryColor;
};
...
const SpotifyTitle = ({ color }: { color?: string }) => {
  ...
  const ink = typeof color === 'string' ? color : appTheme.secondaryColor;
...
const SpotifyArtist = ({
  ...
  const imageUrl = artist.image_url_small ?? artist.image_url_large;

  return (
    <Pressable
      onPress={() => Linking.openURL(spotifyArtistUrl(artist))}
      style={{
        flexDirection: 'row',
        alignItems: 'center',
        gap: 8,
      ...
      {!imageUrl &&
        ...
        >
          <Ionicons
            style={{
              fontSize: 16,
              color: inkColor(appTheme, textStyle),
            }}
            name="musical-notes"
          />
        </View>
